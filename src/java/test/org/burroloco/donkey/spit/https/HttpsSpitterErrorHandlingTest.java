...
public class HttpsSpitterErrorHandlingTest extends DonkeyTestCase implements HasFixtures, OverlaysWeb, Destroyable {
    ...
    public void testServerErrorHandling() {
        ...
        Collection<File> all = files.listFiles(ERROR_DIR, FILE, CAN_READ);
        for (File file : all) {
            String actual = files.readFileToString(file);
            if (expected.equals(actual)) return;
        }
        fail("Can't find the error file in " + ERROR_DIR.getName());
    }
